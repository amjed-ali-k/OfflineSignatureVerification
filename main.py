...
def load_model():
    ...
    model.load_state_dict(
        torch.load("Models/model_large_epoch_20", map_location=device)
    )
    return model
...
def main():
    CREATE_TABLE = """CREATE TABLE IF NOT EXISTS signatures (customer_id TEXT PRIMARY KEY,sign1 BLOB, sign2 BLOB, sign3 BLOB, sign4 BLOB)"""
    cursor = connect_to_db().cursor()
...
def upload():
    ...
    file4 = request.files["uploadedImage4"]
    ...
    try:
        ...
        cursor.execute(
            query, (customer_id, file1.read(), file2.read(), file3.read(), file4.read())
        )
        conn.commit()
...
def verify():
    try:
        ...
        input_image_tensor = convert_to_image_tensor(invert_image(input_image)).view(
            1, 1, 220, 155
        )
